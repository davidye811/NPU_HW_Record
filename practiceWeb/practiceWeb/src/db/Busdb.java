...
public class Busdb {
	...
	public static List<String> getAllBus() throws SQLException, AccountDbFailure {
			Bus curBus;
			Connection dbConn;
			Connection dbConn2;
			dbConn = BusAppDataSource.getConnection();
			...
			dbConn.setAutoCommit(false);
			dbConn2.setAutoCommit(false);
			 List<String> result=getAllBusWithDbConnection(dbConn);
          dbConn.commit();
          dbConn2.commit();
          dbConn.close();
          dbConn2.close();
	...
	private static List<String> getAllBusWithDbConnection(Connection dbConn) throws SQLException, AccountDbFailure {
			String readCurBusSql="select `busid` from `busdb`.`bus`";
		
		try(PreparedStatement readCurBusStmt =dbConn.prepareStatement(readCurBusSql)){
			
			return getAllBusWithSqlStmt(readCurBusStmt);
		}
		
		
	}

	private static List<String> getAllBusWithSqlStmt(PreparedStatement readCurBusStmt) throws SQLException {
		List<String> curBus =new ArrayList<String>();
		try(ResultSet results =readCurBusStmt.executeQuery();){
			while(results.next()){
				curBus.add(results.getString("busid"));
			}
		}
		return curBus;
